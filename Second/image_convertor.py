...
class convertor:
    def __init__(self,BG,sizeOfSheet,allowedChars,style):
        self.BG = BG
    ...
    def letterwrite(self,word):
        if self.gap > self.sizeOfSheet - 95*(len(word)):
            ...
            if(self.style=='First'):
               self._ += 200
            if(self.style=='Second'):
               self._ +=100
        for letter in word:        
            if letter in self.allowedChars:
                if letter.islower():
                    if(self.style=='First'):
                        #k=str(random.randint(1,2))
                        letter+=str(1)
                    elif(self.style=='Second'):
                        letter+='3'
                elif letter.isupper():
                    if(self.style=='First'):
                        letter = letter.lower()
